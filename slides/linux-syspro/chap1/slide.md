...
### 1.4.8 エラー処理 errnoの注意点 1

- コール前にerrnoへ0を代入しておき、コール後に0か否かを確認する必要があります。この種の関数は、エラー発生時にerrnoへ設定する値が0 以外であることが保証されています。
