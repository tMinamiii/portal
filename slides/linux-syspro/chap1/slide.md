...
APIがソースレベルのインタフェースを定義するのに対し、**ABIは特定のアーキテクチャにおけるソフトウェア間の低レベルなバイナリインタフェースを定義したもの** です。 アプリケーションが自身やカーネル、ライブラリと何らかの情報をやり取りする際に使用する方法です。
...
- **ファイル名とinodeの組をリンク（link）と言います。**
  ...
  - カーネルはディレクトリに保存された組をもとに、ファイル名を inode番号へ変換します。

---

### 1.4.1.2 ディレクトリとリンク (ファイル名・ファイルパス解決)

1. ユーザアプリケーションがファイル名を指定しファイルオープンを要求すると、カーネルは対象ファイル名を持つ親ディレクトリをオープンし、指定されたファイル名を検索します。
2. さらにファイル名からinode番号を、inode番号からinodeを特定します。
3. inodeにはファイルに対応するメタデータが保存されており、ファイルの内容を保存したディスク上の位置などの情報が得られます。

実際のシステムには多数のディレクトリが存在するのが常です。ファイル名を与えられた場合に、カーネルはどのように検索する親ディレクトリを特定できるのでしょう?

---

### 1.4.1.2 ディレクトリとリンク (絶対パスのファイル・ディレクトリ)

- ディレクトリと通常ファイルの差異は少なく両者ともに対応するinodeを持っています。
  - このため、ディレクトリ内のリンクから他のディレクトリのinodeを得られます。
  - すなわち、ディレクトリは他のディレクトリ内に入れ子状に存在し、ディレクトリ階層を構成します。

- 絶対パスの場合カーネルはパス名から各ディレクトリエントリ（dentry）をたどり次のエントリを検索します。
  - `/home/blackbeard/landscaping.txt`では、カーネルは `/` から検索を始め、`home` に対応するinodeを特定します。次に `blackbeard` の inode を、最後に`landscaping.txt`のinodeを得ます。

- 相対的な位置から表現したパス（例えば`todo/plunder`）相対パス（relative pathname）の場合
  - カーネルに相対パスが与えられた場合は、カレントディレクトリ（current working directory）からパス解決処理を行います。
  - カレントディレクトリから `todo` という名前のディレクトリを検索し、さらに `plunder` の inodeを入手します。

- Linux カーネルでは **dentry cache** というキャッシュも使用しており、パス解決の結果をメモリ上に蓄えるようにしています。キャッシュにより、同じディレクトリに対する以降のパス解決が高速に行えます。

---

### 1.4.1.2 ディレクトリとリンク (ディレクトリとシステムコール)

- ディレクトリは通常ファイルのように扱えますが、オープンなどの操作がカーネルにより禁止されている点が異なります。
- ディレクトリ操作には専用に用意されているシステムコールを使用します。
- リンクの追加/削除をするシステムコールで、本質的にはこれだけです。
- 仮にカーネルの仲介なしのディレクトリ操作をユーザ空間に許してしまうと、たった1つの単純な誤りでもファイルシステムの破壊につながるような事態が容易に発生してしまうでしょう。
...
- 例えば、あるデータを参照する同じinodeに対し、 `/home/bluebeard/map.txt`、`/home/blackbeard/treasure.txt`という2つの異なるファイル名をハードリンクできます
- ファイルシステム上に他のハードリンクが存在している場合があり得るためです。すべてのリンクが削除されるまでデータを破棄しないために、inodeにはリンクカウント（link count）というデータが存在します。
- リンクカウントはファイルシステム上に存在する現在のリンク数を表します。ファイルを削除すると、リンクカウントは 1 減らされます。リンクカウントが 0 になった場合のみ、inode や対応するデータがファイルシステムから破棄されます。

---

### 1.4.1.4 シンボリックリンク

シンボリックリンクが異なるファイルシステムのファイルをリンクするものだとは知らなかった

- ハードリンクは異なるファイルシステムにまたがることはできません。ファイルシステム間をまたぐリンクは透過性は高くありませんが単純なため、Unixシステムではシンボリックリンク（symbolic link）も備えています。
  - 透過性とは「ユーザが特に意識する必要があるか否かを表現する言葉で、内部ですべ て自動的に処理される場合を透過性が高いと言います。」
- シンボリックリンクも通常ファイルと同様に扱えます。シンボリックリンクは自身のinodeとデータを持ち、シンボリックリンク先のパス名を保持します。この仕組みによりシンボリックリンクではどんな場所にあるものでも参照可能になります。
- 異なるファイルシステムに存在するファイルやディレクトリも参照可能ですし、存在しないものでも参照先として使用できます。存在しないファイルを参照するシンボリックリンクを **broken link** と呼びます。

---

### 1.4.1.4 シンボリックリンク デメリット

- ハードリンクに比べ、シンボリックリンクはオーバヘッドが大きくなります。シンボリックリンクをたどるにはシンボリックリンク自身のパス解決に加え、参照先ファイルのパス解決も必要になるためです。
  - ハードリンクではこのオーバヘッドは発生しません。リンクカウントが1のファイルでも、2以上のファイルでもアクセスに差はありません。
  - シンボリックリンクのオーバヘッドは最低限に抑えられていますが、それでも肯定的にはとらえられていません。

- ハードリンクに比べればシンボリックリンクの透過性‡は高くありません。ハードリンクは完全に透過で、複数リンクされていることも検知できます。
- シンボリックリンクの操作は専用のシステムコールを必要としますが、この非透過性もハードリンクよりも高度なショートカットとして機能するため、許容する向きもあります。

### 1.4.1.5　スペシャルファイル (p12, 13)

- スペシャルファイルはファイルとして表現されたカーネルオブジェクトです。
...
- もちろん Linux ではスペシャルファイルを作成するシステムコールを備えています。
- Unixシステムではデバイスへのアクセスはデバイスファイルを介して行われます。
- デバイスファイルも通常ファイルのようにファイルシステム上に存在するものです。
- デバイスファイルはオープン、読み取り、書き込みが可能で、ユーザ空間からシステムのデバイス（実デバイス、仮想デバイスともに）へのアクセス、操
作が可能です。
- Unixのデバイスは一般的に 2種類に分類されます。キャラクタデバイス（character device）とブロックデバイス（block device）です。デバイスそれぞれに専用のデバイスファイルが存在します。


### 1.4.1.5 スペシャルファイル - キャラクタデバイス (p13)
...
### 1.4.1.5 スペシャルファイル - ブロックデバイス (p13)

- 対称的にブロックデバイスは、データをバイト配列としてアクセスするものです。
- デバイスドライバはシーク可能なデバイスにデータバイトをマッピングし、ユーザ空間からはバイト配列内ならばどのバイトにもアクセス可能です。
- 順序も関係ありません。12番目のバイトを読み取り、次に7番目のバイトを読み取った後でも、再び12番目のバイトへアクセスできます。
...
### 1.4.1.5 スペシャルファイル - ソケット (p13)

- 最後に挙げるスペシャルファイルはソケット（socket）です。
- ソケットはIPCが進化したもので、プロセス間の通信に使用します。
- 同じマシン内のプロセスに限定せず、異なるマシン間でも通信可能です。
- 実際、ソケットはネットワークやインターネットを使用したプログラミングの基本です。
- ソケットにはいろいろな種類があり、Unixドメインのソケットはソケットの形を取っていますが、同一マシン内での通信にしか使用で
きません。
- インターネットを経由したソケット通信はホスト名とポート番号の組を用いて通信先を特定しますが、Unixドメインソケットではファイルシステム上に存在するスペシャルファイルを用いて特定します。
- Unix ドメインソケットは単にソケットファイルとも呼ばれます。
